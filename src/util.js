...
export const getRailroadSection = ( setRailroadSection ) =>{
    ...
    axios.get(URL_ROOT+`/api/railroads/structures`,{
...
export const getInchonSpeedData = ( setTrackSpeedData ) => {
    ...
    axios.all([
        axios.get(URL_ROOT+"/resources/data/railroads/operatingspeed/incheon_1_t1.csv", { responseType: 'text' }),
        axios.get(URL_ROOT+"/resources/data/railroads/operatingspeed/incheon_1_t2.csv", { responseType: 'text' })
      ])
...
export const getSeoulSpeedData = ( setTrackSpeedData ) => {
    ...
    axios.all([
        axios.get(URL_ROOT+"/resources/data/railroads/operatingspeed/seoul_2_t1.csv", { responseType: 'text' }),
        axios.get(URL_ROOT+"/resources/data/railroads/operatingspeed/seoul_2_t2.csv", { responseType: 'text' })
      ])
      .then(axios.spread((response1, response2) => {
        // 두 요청이 모두 완료됐을 때 실행됩니다.
        /* console.log('Data 1:', response1.data);
...
export const dataUploadTitle = (val) => {
    if( val === UPLOAD_CATEGORY_ACCUMULATEWEIGHTS){
        return "누적통과톤수";
    }else if( val === UPLOAD_CATEGORY_RAILWEARS ){
        return "마모유지관리"; 
    }else if( val === UPLOAD_CATEGORY_RAILTWISTS ){
        return "궤도틀림";
    }else if( val === UPLOAD_CATEGORY_RAILBEHAVIORS ){
        return "궤도거동계측";
    }else if( val === UPLOAD_CATEGORY_RAILROUGHNESS ){
        return "레일조도";
    }else if( val === UPLOAD_CATEGORY_RAILSTRAIGHTS ){
        return "레일직진도";
    }else if( val === UPLOAD_CATEGORY_RAILPROFILES ){
        return "레일프로파일";
    }
    return "";
}

export const checkUniqueness = (arr) => {
    // 모든 키의 값을 담을 배열을 준비합니다.
    let values = [];
